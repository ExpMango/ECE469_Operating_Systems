...
Link *AQueueAllocLink(void *obj_to_store)
{
  ...
  if (freeLinks.first == l)
    freeLinks.first = l->next;
  if (freeLinks.last == l)
    freeLinks.last = l->prev;
  freeLinks.nitems--;
...
int AQueueRemove(Link **pl)
{
  ...
  if (AQueueFirst(l->queue) == l)
    l->queue->first = l->next; // l was first item on queue
  if (AQueueLast(l->queue) == l)
    l->queue->last = l->prev; // l was last item on queue
...
int AQueueInsertBefore(Queue *q, Link *before, Link *l)
{
  ...
  if (before)
  { //  If before is not NULL, then before must be on the queue q
    if (before->queue != q)
      return QUEUE_FAIL;
  }
  else
  { // Otherwise before is NULL, so the list has to be empty
    if (AQueueFirst(q) != NULL)
      return QUEUE_FAIL;
    if (AQueueLast(q) != NULL)
      return QUEUE_FAIL;
    if (!AQueueEmpty(q))
      return QUEUE_FAIL;
  }
...
int AQueueMoveAfter(Queue *q, Link *after, Link *l)
{
  ...
  if (!l)
    return QUEUE_FAIL;
  if (l->queue != q)
    return QUEUE_FAIL; // l does not belong to q
  ...
  if (q->last == l)
    return QUEUE_FAIL; // this would be strange if it happened
  if (q->first == l)
    q->first = l->next;
  if (q->last == after)
    q->last = l;
