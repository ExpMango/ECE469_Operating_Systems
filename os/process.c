...
// to happen).
static Queue runQueue[RUNQUEUE_NUM];
...
void ProcessModuleInit()
{
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++)
  {
    ...
    {
      printf("FATAL ERROR: could not insert PCB link into queue in "
             "ProcessModuleInit!\n");
      exitsim();
    }
  }
...
void ProcessFreeResources(PCB *pcb)
{
  ...
  {
    printf("FATAL ERROR: could not close mbox!\n");
    exitsim();
  }
  ...
  {
    printf("FATAL ERROR: could not insert PCB link into freepcbs queue in "
           "ProcessFreeResources!\n");
    exitsim();
  }
...
void ProcessSchedule()
{
  ...
  int i = 0;
  Link *l = NULL;
  int have_runable_process = 0;
  int run_jiffies = 0;
  Queue *curr_queue;

  dbprintf('p', "Now entering ProcessSchedule (cur=0x%x, %d ready)\n",
           (int)currentPCB, AQueueLength(&runQueue[0]));
  ...
  for (i = 0; i < RUNQUEUE_NUM; ++i)
  {
    if (!AQueueEmpty(&runQueue[i]))
      have_runable_process = 1;
  }

  if (!have_runable_process)
  {
    if (!AQueueEmpty(&waitQueue))
    {
      printf("FATAL ERROR: no runnable processes, but there are sleeping "
             "processes waiting!\n");
  ...
  // count jiffies
  run_jiffies = ClkGetCurJiffies() - currentPCB->process_start_time;
  currentPCB->jiffies_count += run_jiffies;
  if (run_jiffies >= JIFFIES_PER_WINDOW)
    currentPCB->estcpu += 1.0;
  ProcessRecalcPriority(currentPCB);

  quantum += run_jiffies;
  if (quantum >= 10 * JIFFIES_PER_WINDOW)
  {
  ...
  // front.
  curr_queue = &runQueue[WhichQueue(currentPCB)];
  AQueueMoveAfter(&curr_queue, AQueueLast(&curr_queue), AQueueFirst(&curr_queue));
  ...
  // pcb changed, update process_start_time
  pcb->process_start_time = ClkGetCurJiffies();
  ...
  while (!AQueueEmpty(&zombieQueue))
  {
    ...
    {
      printf("FATAL ERROR: could not remove zombie process from zombieQueue in "
             "ProcessSchedule!\n");
      exitsim();
    }
...
void ProcessSuspend(PCB *suspend)
{
  ...
  {
    printf("FATAL ERROR: could not remove process from run Queue in "
           "ProcessSuspend!\n");
    exitsim();
  }
...
void ProcessWakeup(PCB *wakeup)
{
  ...
  {
    printf("FATAL ERROR: could not remove wakeup PCB from waitQueue in "
           "ProcessWakeup!\n");
    exitsim();
  }
                ...
                int isUser)
{
  ...
  if (AQueueRemove(&(pcb->l)) != QUEUE_SUCCESS)
  {
    printf("FATAL ERROR: could not remove link from freepcbsQueue in "
           "ProcessFork!\n");
    exitsim();
  }
  ...
  stackframe[PROCESS_STACK_PTBITS] =
      (MEMORY_L1_PAGE_SIZE_BITS + (MEMORY_L2_PAGE_SIZE_BITS << 16));
  ...
  else
  {
    ...
    stackframe[PROCESS_STACK_IREG + 29] =
        pcb->sysStackArea + MEMORY_PAGE_SIZE - 32;
...
void main(int argc, char *argv[])
{
  ...
  if (userprog != (char *)0)
  {
    ...
    case 4:
      ...
      break;
    case 5:
      process_create(params[0], params[1], params[2], params[3], params[4],
                     NULL);
      break;
    case 6:
      process_create(params[0], params[1], params[2], params[3], params[4],
    ...
    case 7:
      process_create(params[0], params[1], params[2], params[3], params[4],
                     params[5], params[6], NULL);
      break;
    case 8:
      process_create(params[0], params[1], params[2], params[3], params[4],
                     params[5], params[6], params[7], NULL);
      break;
    case 9:
      process_create(params[0], params[1], params[2], params[3], params[4],
                     params[5], params[6], params[7], params[8], NULL);
      break;
    case 10:
      process_create(params[0], params[1], params[2], params[3], params[4],
...
unsigned GetCurrentPid() { return (unsigned)(currentPCB - pcbs); }

unsigned findpid(PCB *pcb) { return (unsigned)(pcb - pcbs); }
...
int GetPidFromAddress(PCB *pcb) { return (int)(pcb - pcbs); }
...
{
  if (pcb->flags & PROCESS_TYPE_SYSTEM)
  {
    pcb->priority = KERNEL_BASE_PRIORITY + pcb->estcpu / 4 + 2 * pcb->pnice;
  }
  else
  {
    pcb->priority = USER_BASE_PRIORITY + pcb->estcpu / 4 + 2 * pcb->pnice;
  }
}
...
PCB *ProcessFindHighestPriorityPCB()
{
  ...
  for (i = 0; i < RUNQUEUE_NUM; i++)
  {
    if (!AQueueEmpty(&runQueue[i]))
    {
      return (PCB *)AQueueObject(AQueueFirst(&runQueue[i]));
    }
  }
  return NULL;
}